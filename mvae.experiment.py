...
"""
MNIST Autoencoder with ASH layers
"""
...
def go(batch=64, epochs=350, k=750, additional=512, modelname='baseline', cuda=False, seed=1, bias=True, lr=0.001, lambd=0.01, data='./data',):
    ...
    for epoch in range(epochs):
        for i, data in tqdm(enumerate(trainloader, 0)):
            ...
            rec_loss_encoder = mse(r[0], inputs) + mse(r[1], inputs) + mse(r[2], inputs)
            rec_loss_decoder = mse(r[3], sample_target) + mse(r[4], sample_target)

            vae = vae_loss(inputs, reconstruction, mu, logvar)
            ...
            if PLOT and i == 0:
                ...
                plt.savefig('mvae.{:03d}.input.pdf'.format(epoch))
                ...
                for i, recon in enumerate(r[:3]):
                    ...
                    plt.imshow(np.transpose(torchvision.utils.make_grid(recon.data[:16, :]).cpu().numpy(), (1, 2, 0)),
                               interpolation='nearest')
                    plt.savefig('mvae.{:03d}.layer{:01d}.pdf'.format(epoch, i))
                ...
                plt.imshow(np.transpose(torchvision.utils.make_grid(reconstruction.data[:16, :]).cpu().numpy(), (1, 2, 0)),
                           interpolation='nearest')
                plt.savefig('mvae.{:03d}.output.pdf'.format(epoch))
...
if __name__ == "__main__":
    ...
    go(batch=options.batch_size, k=options.k, bias=options.bias,
