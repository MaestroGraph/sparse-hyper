...
class SortLayer(gaussian_temp.HyperLayer):
    ...
    """
    def __init__(self, size, k, learn=1, additional=0, sigma_scale=0.1, fix_values=False, sigma_floor=0.0):
        ...
        super().__init__(in_rank=1, out_size=(size,),
                         temp_indices=temp, learn_cols=(learn,),
                         additional=additional, bias_type=gaussian.Bias.NONE, subsample=None)
    ...
    def hyper(self, input):
        ...
        res = self.source(input).unsqueeze(2).view(b, self.k, 3)
...
def go(arg):
    ...
    for r in range(arg.reps):
        ...
        util.makedirs('./sort-temp/{}'.format( r))
        SHAPE = (arg.size,)
        ...
        for i in trange(arg.iterations):
            ...
            w.add_scalar('sort-temp/loss/{}/{}'.format(arg.size, r), loss.data.item(), i*arg.batch_size)
            ...
            if i % arg.dot_every == 0:
                ...
                w.add_scalar('sort-temp/accuracy/{}/{}'.format(arg.size, r), correct/tot, i * arg.batch_size)
            ...
            if i % arg.plot_every == 0:
                ...
                plt.savefig('./sort-temp/{}/means{:04}.pdf'.format(r, i))

    np.save('results-temp.{}.np'.format(arg.size), results)
...
#    ax.set_xlim(0.0, 100.0)
    ...
    plt.savefig('./sort-temp/result.png')
    plt.savefig('./sort-temp/result.pdf')
